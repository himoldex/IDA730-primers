...
```{r r1-check}
"You can now identify which values contain more observations. The darker locations contain several points stacked on top of each other."
```
...
```{r r2-check}
"Now let's look at ways to handle overplotting due to large datasets."
```
...
```{r r3-check}
"The medians of the boxplots give a somewhat more precise description of the relationship between carat and price than does the fan of individual points."
```
...
```{r r4-check}
"As with histograms, bin2ds can reveal different information at different binwidths."
```
...
```{r r5-check}
"You need to have the hexbin package installed on your computer, but not necessarily loaded, to use geom_hex()."
```
...
```{r r6-check}
"Plotting a summay on top of raw values is a common pattern in data science."
```
