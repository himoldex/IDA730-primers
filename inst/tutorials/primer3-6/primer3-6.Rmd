...
```{r r1-solution, exercise.reveal_solution = TRUE}
ggplot(economics) +
  geom_line(mapping = aes(x = date, y = unemploy))
```

```{r r1-check}
"The graph shows the number of unemployed people in the US (in thousands) from 1967 to 2015. Now let's look at a more rich dataset."
```
...
```{r r3-solution, exercise.reveal_solution = TRUE}
ggplot(asia) +
  geom_point(mapping = aes(x = year, y = gdpPercap))
```

```{r r3-check}
"There are actually four lines in the plot. One for each country: China, Japan, North Korea, and South Korea."
```
...
```{r r4-solution, exercise.reveal_solution = TRUE}
ggplot(asia) +
  geom_line(mapping = aes(x = year, y = gdpPercap, group = country))
```

```{r r4-check}
"We now have a separate line for each country. Unfortunately, we cannot tell what the countries are: the group aesthetic does not supply a legend. Let's look at how to fix that."
```
...
```{r r5-solution, exercise.reveal_solution = TRUE}
ggplot(asia) +
  geom_line(mapping = aes(x = year, y = gdpPercap, linetype = country, color = country))
```

```{r r5-check}
"If you map two aesthetics to the same variable, ggplot2 will combine their legends. Supplementing color with linetype is a good idea if you might print your line chart in black and white."
```
...
```{r r6-solution, exercise.reveal_solution = TRUE}
ggplot(asia) +
  geom_line(mapping = aes(x = year, y = lifeExp, color = country, linetype = country))
```

```{r r6-check}
"Japan has the highest life expectancy and North Korea the worst, but we can see that things haven't always been this way. Now let's look at some other ways to display the same information."
```
...
```{r r7-solution, exercise.reveal_solution = TRUE}
ggplot(asia) +
  geom_step(mapping = aes(x = year, y = lifeExp, color = country, linetype = country))
```

```{r r7-check}
'You can control whether the steps move horizontally first and then vertically or vertically first and then horizontally with the parameters `direction = "hv"` (the default) or `direction = "vh"`.'
```
...
```{r r0-solution, exercise.reveal_solution = TRUE}
ggplot(economics) +
  geom_area(mapping = aes(x = date, y = unemploy))
```

```{r r0-check}
"Filling the space under the line gives you a new way to customize your plot."
```
...
```{r r8-solution, exercise.reveal_solution = TRUE}
ggplot(economics) +
  geom_area(mapping = aes(x = date, y = unemploy), fill = "blue")
```

```{r r8-check}
"Remember that you map aesthetics to variables inside of aes(). You set aesthetics to values outside of aes()."
```
...
```{r r9-solution, exercise.reveal_solution = TRUE}
ggplot(asia) +
  geom_area(mapping = aes(x = year, y = lifeExp, fill = country), position = "identity", alpha = 0.3)
```

```{r r9-check}
"You can further customize your graph by switching from `geom_area()` to `geom_ribbon()`. `geom_ribbon()` lets you map the bottom of the filled area to a variable, as well as the top. See `?geom_ribbon` if you'd like to learn more."
```
...
```{r r10-solution, exercise.reveal_solution = TRUE}
ggplot(tx) +
  geom_path(mapping = aes(x = long, y = lat))
```

```{r r10-check}
"`geom_path()` reveals how you can use what is essentially a line plot to make a map (this is a map of the state of Texas). There are other ways to make maps in R, but this low tech method is surprisingly versatile."
```
...
```{r r12-solution, exercise.reveal_solution = TRUE}
...
```{r r12-check}
"This is how I collected the tx dataset. Let's look at what would happen if you do not specify a region in map_data()."
```
...
```{r r13-solution, exercise.reveal_solution = TRUE}
ggplot(us) +
  geom_polygon(mapping = aes(x = long, y = lat, group = group))
```

```{r r13-check}
"You've mastered a method for drawing maps. Let's take the final step and use the map to display information."
```
...
```{r r15-check}
"You've used geom_map() to make your first choropleth plot! To test your understanding, alter the code to display the Murder, Assault, or Rape variables."
```
...
```{r r16-solution, exercise.reveal_solution = TRUE}
ggplot(USArrests2) +
  geom_map(aes(map_id = region, fill = UrbanPop), map = us) +
  expand_limits(x = us$long, y = us$lat) +
...
```{r r16-check}
"To see a list of available projections, visit the `?mapproj` help page, which is hyperlinked from the coord_map() help page."
```
