...
```{r ex1-solution, exercise.reveal_solution = TRUE}
flights %>% 
  drop_na(arr_delay) %>% 
  group_by(carrier) %>% 
...
```{r ex1-check}
"You've calculated the average delay per airline, but the results are difficult to interpret. We don't know which codes are associated with which airlines."
```
...
```{r out.width = "60%"}
knitr::include_graphics("instrument.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("left-join.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("right-join.png")
```
...
```{r ex4-solution, exercise.reveal_solution = TRUE}
instrument %>% left_join(band, by = "name")
```

```{r ex4-check}
"Since right and left joins are analagous, you can acheive the same results by switching the order of the data sets in a left join. Notice that this will affect the column order."
```
...
```{r out.width = "90%"}
knitr::include_graphics("full-join.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("inner-join.png")
```
...
```{r ex7-solution, exercise.reveal_solution = TRUE}
flights %>% 
  drop_na(arr_delay) %>% 
  group_by(carrier) %>% 
  summarise(avg_delay = mean(arr_delay)) %>% 
...
```{r ex7-check}
"It is now easy to see that Frontier Airlines, AirTran, and ExpressJet had the longest average delays departing New York City in 2013."
```
...
```{r out.width = "80%"}
knitr::include_graphics("nycflights13.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("match-names.png")
```
...
```{r ex10-solution, exercise.reveal_solution = TRUE}
flights %>% 
  drop_na(arr_delay) %>%
  group_by(dest) %>%
  summarise(avg_delay = mean(arr_delay)) %>%
...
```{r ex10-check}
"Flights from NYC to Columbia, South Carolina seem to have arrived particularly late in 2013. At the other end of the list, far off destinations in Alaska and Hawaii tended to arrive ahead of schedule."
```
...
```{r out.width = "90%"}
knitr::include_graphics("semi-join.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("anti-join.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("distinct.png")
```
...
```{r ex14-solution, exercise.reveal_solution = TRUE}
flights %>% 
  distinct(dest)
```

```{r ex14-check}
"There are 105 distinct destinations in the flights data set."
```
...
```{r ex15-solution, exercise.reveal_solution = TRUE}
flights %>% 
  distinct(dest) %>% 
  left_join(airports, by = c("dest" = "faa")) %>% 
  select(name)
...
```{r ex15-check}
"The third result is an NA. What's going on?"
```
...
```{r ex16-solution, exercise.reveal_solution = TRUE}
flights %>%
  distinct(dest) %>% 
...
```{r ex17-solution, exercise.reveal_solution = TRUE}
flights %>% 
  semi_join(airports, by = c("dest" = "faa")) 
```

```{r ex17-check}
"Most of the rows in flights are associated with a known airport."
```
...
```{r ex18-solution, exercise.reveal_solution = TRUE}
flights %>% 
...
```{r out.width = "50%"}
knitr::include_graphics("beatles.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("bind-cols.png")
```
...
```{r out.width = "50%"}
knitr::include_graphics("band2.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("bind-rows.png")
```
...
```{r out.width = "90%"}
knitr::include_graphics("bind-rows-id.png")
```
...
```{r ex21-solution, exercise.reveal_solution = TRUE}
bands <- list(df1 = band, 
              df2 = band2)
...
```{r ex21-check}
"You can add more than two data sets to your list if you wish to bind together multiple data sets at once."
```
...
```{r out.width = "90%"}
knitr::include_graphics("setdiff.png")
```
