...
```{r functions-1, exercise = TRUE, exercise.eval = FALSE}
# Write you solution here
```

```{r functions-1-solution, exercise.reveal_solution = TRUE}
sqrt(961)
```
...
```{r functions-2-solution, exercise.reveal_solution = TRUE}
sqrt
```

```{r functions-2-check}
"`sqrt` immediately triggers a low level algorithm optimized for performance, so there is not much code to see."
```
...
```{r functions-3-solution, exercise.reveal_solution = TRUE}
lm
```

### help pages

`lm()` runs a lot of code. What does it do? Open the help page for `lm()` in the chunk below and find out.

```{r functions-4, exercise = TRUE}
# Write you solution here
```

```{r functions-4-solution, exercise.reveal_solution = TRUE}
?lm
```

```{r functions-4-check}
"`lm()` is R's function for fitting basic linear models. No wonder it runs so much code."
```
...
```{r functions-5-solution, exercise.reveal_solution = TRUE}
#sqrt(961)
```
...
```{r arguments-1-solution, exercise.reveal_solution = TRUE}
args(rnorm)
```

```{r arguments-1-check}
"`n` specifies the number of random normal variables to generate. `mean` and `sd` describe the distribution to generate the random values with." 
```
...
```{r arguments-3-solution, exercise.reveal_solution = TRUE}
...
```{r arguments-4-solution, exercise.reveal_solution = TRUE}
rnorm(100, mean = 100, sd = 50)
```
...
```{r objects-2-solution, exercise.reveal_solution = TRUE}
...
```{r objects-3-solution, exercise.reveal_solution = TRUE}
data <- rnorm(100, mean = 100, sd = 15)
copy <- data
...
```{r objects-3-check}
"R saves a copy of the contents in data to copy."
```
...
```{r objects-4-solution, exercise.reveal_solution = TRUE}
iris
```

```{r objects-4-check}
"You can learn more about iris by examining its help page with `?iris`."
```
...
```{r objects-5-solution, exercise.reveal_solution = TRUE}
iris <- 1
iris
...
```{r objects-5-check}
"Unfortunately, `rm()` cannot help you if you overwrite one of your own objects."
```
...
```{r vectors-1-solution, exercise.reveal_solution = TRUE}
...
```{r vectors-2-solution, exercise.reveal_solution = TRUE}
1:10
```
...
```{r vectors-3-solution, exercise.reveal_solution = TRUE}
vec <- c(1, 2, 4, 8, 16)
...
```{r vectors-4-solution, exercise.reveal_solution = TRUE}
vec <- c(1, 2, 4, 8, 16)
vec[c(1,2,5)]
```
...
```{r vectors-5-solution, exercise.reveal_solution = TRUE}
vec2 <- c(alpha = 1, beta = 2, gamma = 3)
...
```{r vectors-6-check}
"Like many R functions, R's math operators are vectorised: they're designed to work with vectors by repeating the operation for each pair of elements."
```
...
```{r vectors-7-check}
"Whenever you try to work with vectors of varying lengths (recall that `1` is a vector of length one), R will repeat the shorter vector as needed to compute the result."
```
...
```{r types3-solution, exercise.reveal_solution = TRUE}
c(1L, 2L, 3L, 4L, 5L)
```
...
```{r types4-solution, exercise.reveal_solution = TRUE}
sqrt(2) * sqrt(2) - 2
sqrt(2)^2 - 2
```
...
```{r lists2-solution, exercise.reveal_solution = TRUE}
...
```{r lists4-solution, exercise.reveal_solution = TRUE}
nums <- c(1, 2, 3, 4)
logs <- c(TRUE, TRUE, FALSE, TRUE)
strs <- c("apple", "banana", "carrot", "duck")
...
```{r lists5-solution, exercise.reveal_solution = TRUE}
df$strings
```
...
```{r packages2-solution, exercise.reveal_solution = TRUE}
library(tidyverse)
```

```{r packages2-check}
"R will keep the packages loaded until you close your R session. When you re-open R, you'll need to reload you packages."
```
...
```{r packages3-solution, exercise.reveal_solution = TRUE}
install.packages("dplyr")
```

```{r packages3-check}
"You only need to install a package once, unless you wish to update your local copying by reinstalling the package. Notice that `install.packages()` _always_ requires quotes around the package name."
```
