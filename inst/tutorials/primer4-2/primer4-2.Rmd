...
```{r ex1-check}
"As with other tidyverse functions, `separate()` returns a modified copy of the orginal data. You will need to save the copy if you wish to use it later."
```
...
```{r ex2-check}
'"-" is the only non-alphanumeric character used in our dates, which means that the defaults return the same output as setting sep = "-"'
```
...
```{r ex3-solution, exercise.reveal_solution = TRUE}
hurricanes %>% 
  separate(col = date, into = c("year","month","day")) %>% 
  separate(col = year, into = c("century", "year"), sep = 2)
```
...
```{r ex3-check}
"Notice that when you separate on a character, separate() removes that character from the results. When you separate on a location, separate() retains all of the characters in the result."
```
...
```{r ex6-solution, exercise.reveal_solution = TRUE}
hurricanes %>% 
  separate(col = date, into = c("year","month","day"), convert = TRUE)
```

```{r ex6-check}
"Our columns now contain integers, which will make them easier to manipulate with mathematical operations."
```
...
* month/day/year, e.g., 1/27/2021

```{r ex8, exercise = TRUE}
# type code below this line
```

```{r ex8-solution, exercise.reveal_solution = TRUE}
hurricanes %>%
  separate(date, c("year", "month", "day"), sep = "-") %>%
  unite(col = "date", month, day, year, sep = "/")
```
...
```{r ex8-check}
"Let's push it one step farther."
```
...
```{r ex9-solution, exercise.reveal_solution = TRUE}
hurricanes %>%
...
```{r out.width = "90%"}
knitr::include_graphics("codes.png")
```
...
```{r ex10-check}
"The - helper becomes very handy when you have 60 column names to deal with!"
```
...
```{r ex11-solution, exercise.reveal_solution = TRUE}
who %>%
  select(-iso2, -iso3) %>% 
...
```{r ex11-check}
"The codes are easier to work with when they are in their own column."
```
...
```{r ex12-solution, exercise.reveal_solution = TRUE}
who %>%
  select(-iso2, -iso3) %>% 
  gather(key = "codes", value = "n", 3:58) %>% 
...
```{r ex12-check}
"Luckily, it is easy to decompose each code into its components by separating on the _ characters. However, sex and age still appear in the same column."
```
...
```{r ex13-solution, exercise.reveal_solution = TRUE}
who %>%
  select(-iso2, -iso3) %>% 
  gather(key = "codes", value = "n", 3:58) %>% 
  separate(codes, into = c("new", "type", "sexage"), sep = "_") %>% 
...
```{r ex13-check}
"We've almost finished tidying this data set. Let's add some finishing touches."
```
...
```{r ex14-check}
"Now let's turn our attention to the NA's in the n column."
```
...
```{r ex15-check}
"You may have noticed that drop_na(n) does the same thing as filter(!is.na(n)). However, drop_na(n) is easier to read and type."
```

### Recap

You've wrangled `who` into a tidy, polished data set that is ready to be explored, modelled, and analyzed.
