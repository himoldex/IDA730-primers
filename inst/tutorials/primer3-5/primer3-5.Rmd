...
```{r r1-check}
"It can be tricky to remember when to use %>% and when to use +. Use %>% to add one complete step to a pipe of code. Use + to add one more line to a ggplot2 call."
```
...
```{r r2-check}
"geom_text() replaces each point with a piece of text supplied by the label aesthetic. geom_label replaces each point with a textbox. Notice that some pieces of text overlap each other, and others run off the page. We'll soon look at a way to fix this."
```
...
```{r r3-check}
"Now let's look at a way to make geom_smooth() much more useful."
```
...
```{r r4-check}
"You can add as many geom functions as you like to a plot; but, in practice, a plot will become hard to interpret if it contains more than two or three geoms."
```
...
```{r r5-check}
"The ggrepel package also provides geom_text_repel(), which is an analog for geom_text()."
```
...
```{r r6-check}
"Remember that not every mapping should be a global mapping. Here, only geom_label_repel uses the label aesthetic. Hence, it should remain a local aesthetic to avoid unintended side effects, warnings, or errors."
```
...
```{r r10-check}
'Pass geom_rug() the parameter sides = "l" to limit the rug plot to just the left-hand axis (y) or sides = "b" to limit the rug plot to just the bottom axis (x).'
```
...
```{r r11-check}
"You can also jitter in only a single direction. To turn off jittering in the x direction set width = 0 in geom_jitter(). To turn off jittering in the y direction, set height = 0." 
```
...
```{r r12-check}
"If you like, you can make the boxplots more visible by setting the alpha parameter of geom_jitter() to a low number, e.g. geom_jitter(mapping = aes(x = class, y = hwy), width = 0, alpha = 0.5)"
```
...
```{r r13-check}
"Now let's look at how you can do the same transformation, and others as well with a coord function."
```
