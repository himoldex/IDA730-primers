...
It is natural to begin with the entire data set, which if plotted as a scatterplot would've looked like this.
...
```{r select-1-solution, exercise.reveal_solution = TRUE}
...
```{r filter-0-solution, exercise.reveal_solution = TRUE}
...
```{r filter-1a-solution, exercise.reveal_solution = TRUE}
filter(babynames, name == "Sea")
```

```{r filter-1a-check}
"Remember to use == instead of = when testing for equality."
```
...
```{r filter-1b-solution, exercise.reveal_solution = TRUE}
filter(babynames, name == "Sea")
```

```{r filter-1b-check}
"As written this code would check that name is equal to the contents of the object named Sea, which does not exist."
```
...
```{r filter-2-solution, exercise.reveal_solution = TRUE}
...
```{r filter-3-solution, exercise.reveal_solution = TRUE}
filter(babynames, 10 < n, n < 20)
```

```{r filter-3-check}
"You cannot combine two logical tests in R without using a Boolean operator (or at least a comma between filter arguments)."
```
...
```{r filter-4-solution, exercise.reveal_solution = TRUE}
filter(babynames, n %in% c(5, 6, 7, 8, 9))
```

```{r filter-4-check}
"Although the first code works, you should make your code more concise by collapsing multiple or statements into an %in% statement when possible."
```
...
```{r arrange-1-solution, exercise.reveal_solution = TRUE}
arrange(babynames, n)
```

```{r arrange-1-check}
"The compiler of `babynames` used 5 as a cutoff; a name only made it into babynames for a given year and gender if it was used for five or more children."
```
...
```{r arrange-3-solution, exercise.reveal_solution = TRUE}
...
```{r arrange-4-solution, exercise.reveal_solution = TRUE}
...
```{r arrange-5-solution, exercise.reveal_solution = TRUE}
...
```{r pipe-1-solution, exercise.reveal_solution = TRUE}
babynames %>% 
...
```{r your-name-1-solution, exercise.reveal_solution = TRUE}
babynames %>% 
  filter(name == "Garrett", sex == "M") %>% 
  select(year, prop) %>% 
  ggplot() +
