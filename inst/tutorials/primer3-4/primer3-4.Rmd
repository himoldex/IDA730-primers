...
knitr::include_graphics("box-png.png")
```
...
```{r outliers-solution, exercise.reveal_solution = TRUE}
ggplot(data = diamonds) +
...
```{r cut-solution, exercise.reveal_solution = TRUE}
cut_width(continuous_vector, width = 2)
```

```{r cut-check}
"Now let's apply the cut functions to our graph."
```
...
```{r r4-check}
"You can now see a relationship between price and carat. You could also make a scatterplot of these variables, but in this case, it would be a black mass of 54,000 data points."
```
...
```{r r6-check}
'When you set `stackdir = "center"`, `geom_dotplot()` arranges each row of dots symmetrically around the $x$ value. This layout will help you understand the next geom. As in the histogram tutorial, it takes a lot of tweaking to make a dotplot look right. As a result, I tend to only use them when I want to make a point.'
```
...
```{r r7-check}
'Another way to interpret a violin plot is to mentally "push" the width of each violin all to one side (so the other side is a straight line). The result would be a density (e.g. `geom_density()`) turned on its side for each distribution).'
```
...
```{r r8-check}
'Can you predict how you would use `draw_quantiles` to draw a horizontal line at a different percentile, like the 60th percentile?.'
```
